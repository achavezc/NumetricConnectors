...
var syncData = function syncData(lastUpdated) {
    ...
    return numetricCon.getDataSetNumetric().then(function(currentListDataset) {
        return syncDataEvents(lastUpdated, currentListDataset).then(function(resultEvents) {
            var result = false;
...
var syncDataEvents = function syncDataEvents(lastUpdated, currentListDataset) {
    ...
    return mixPanelData.getEvents(lastUpdated).then(function(resultEvents) {
        if (resultEvents.Result.Success) {
            ...
            if (resultEvents.Result.Data.length > 0) {
                ...
                return numetricDataConnectorLogic.verifyCreateManyDatasetNumetric(datasetNames, currentListDataset, datasetMixPanel.DataSetList).then(function(resultVerify ) {
                    ...
                    return mixPanelCon.updateRowsMixPanel(resultEvents.Result).then(function(results) {
